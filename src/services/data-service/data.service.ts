...
export class DataService {
  ...
  getUVLevel(uvIndex: number): UvLevel {
    if (uvIndex <= 2) return { level: "Bajo", color: "bg-green-500", textColor: "text-green-700" };
    if (uvIndex <= 5) return { level: "Moderado", color: "bg-yellow-500", textColor: "text-yellow-700" };
    if (uvIndex <= 7) return { level: "Alto", color: "bg-orange-500", textColor: "text-orange-700" };
    if (uvIndex <= 10) return { level: "Muy Alto", color: "bg-red-500", textColor: "text-red-700" };
    return { level: "Extremo", color: "bg-purple-500", textColor: "text-purple-700" };
  }
